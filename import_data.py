...
def load_transport_times(gdf, path_data, center, option_center):
    ...
    def load_transport_data(mode):
        i = 100
        travel_time_matrix = np.load(path_data + "/travel_time_matrix_poly_" + mode + "_" + str(i) + "_" + option_center + ".npy", allow_pickle= True) #"tt_" + center + 
    
        while i < len(gdf) - 100:
            i = i + 100
            temp = np.load(path_data + "/travel_time_matrix_poly_" + mode + "_" + str(i)  + "_" + option_center + ".npy", allow_pickle= True) #"tt_" + center + 
            travel_time_matrix = np.concatenate((travel_time_matrix, temp), axis=0)
        
        temp = np.load(path_data + "/travel_time_matrix_poly_" + mode +  "_" + str(len(gdf))  + "_" + option_center +  ".npy", allow_pickle= True) #"tt_" + center + 
        travel_time_matrix = np.concatenate((travel_time_matrix, temp), axis=0)
